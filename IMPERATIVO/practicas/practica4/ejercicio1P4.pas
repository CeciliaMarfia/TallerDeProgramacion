...
de unidades vendidas y el monto total. }
  ...
  begin
	v.codigoVenta:=random(6)-1;
    write ('Ingrese codigo de venta: ',v.codigoVenta);
    //readln (v.codigoVenta);
    If (v.codigoVenta <> -1) then begin
           v.codigoProducto:=random(10)+1;
           write ('Ingrese codigo de producto: ');
          // readln (v.codigoProducto);
           v.cantUnidades:=(10)+1;
           write ('Ingrese cantidad de unidades: ',v.cantUnidades);
           //readln (v.cantUnidades);
           v.precioUnitario:=random(50)+1;
           write ('Ingrese precio unitario: ',v.precioUnitario);
...
{Contenga un módulo que reciba la estructura generada en el punto a y retorne el código de producto con mayor cantidad de unidades vendidas.}
  ...
  procedure CodigoDeProductoConMayorCantidadDeUnidadesVendidas (a: arbol; var maxCant: integer; var maxCod: integer);
	procedure actualizarMaximo(a:arbol;var maxCant, maxCod:integer);
	begin
  ...
  begin
		if (a <> nil) then begin
			actualizarMaximo(a,maxCant,maxCod);
  ...
  begin
    if (a= nil) then
		...
		if (a^.dato.codigo < unCodigo) then
			ObtenerCantidadCodigosMenores:= ObtenerCantidadCodigosMenores(a^.HD, unCodigo) + ObtenerCantidadCodigosMenores(a^.HI, unCodigo)+1
		else
			ObtenerCantidadCodigosMenores:= ObtenerCantidadCodigosMenores(a^.HD, unCodigo) + ObtenerCantidadCodigosMenores(a^.HI, unCodigo);
  end;
